...
public final class PatchPasswordRequest {
    ...
    @Pattern(regexp = REGEX_PASSWORD, message = "Invalid character(s) in the current password, every character must be letter and the length must be at least 8 characters")
    @NotNull(message = "Current password cannot be null")
    private final String currentPassword;

    @Pattern(regexp = REGEX_PASSWORD, message = "Invalid character(s) in the new password, every character must be letter and the length must be at least 8 characters")
